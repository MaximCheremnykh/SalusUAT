//npx playwright test tests/smokeTest/02.3-Bogart-tab-dashboard.spec.ts --project=chromium --headed
...
const METRICS: MetricVariants[] = [
  ...
  ["P0 PAX Referred to DHS"],
  ["P2 PAX Initial Outreach"],
  ["P3 PAX Processing"],
  ["P4 PAX Plane Ticketing"],
  ["P5 PAX Departure"], 
  ["P6 PAX Departed"],
  ["P7 PAX Resettlement Stipend"],
];
...
test.describe("@smokeTest CSRO Dashboard (Volunteer) : Bogart tab", () => {
  test("Bogart: refresh + verify metrics", async ({ page }) => {
...
await step("Switch to Bogart and verify selected", async () => {
  ...
  await dash.waitForMetricsHeading(); 
    ...
    await step("Verify CSRO Dashboard - Bogart header visible", async () => {
    ...
    await step("Verify 'PAX Traveler Status Metrics - Bogart' is visible", async () => {
      const frame = await dash.getDashboardFrame();
      await expect(frame.getByText("PAX Traveler Status Metrics - Bogart", { exact: true }))
