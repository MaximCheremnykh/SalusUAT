...
test.describe("@smokeTest CSRO Dashboard (Volunteer) : Bluebird tab", () => {
  test("Bluebird: header, refresh x2 with toast, metrics heading, click tile", async ({ page }) => {
    const dash = new DashboardPage(page);

    await step("Open dashboard via Home", async () => {
      await dash.openDashboard(); // defaults to Bluebird in the page object
    ...
    await step("Switch to Bluebird and verify selected", async () => {
      ...
      await dash.verifyBluebirdSelected();
    });

    await step("Verify Bluebird dashboard header visible", async () => {
      await dash.verifyDashboardTitle("CSRO Dashboard - Bluebird");
    });
