//npx playwright test tests/smokeTest/02.2-Tadpole-tab-dashboard.spec.ts --project=chromium --headed
...
test.describe("@smokeTest CSRO Dashboard (Volunteer) : Tadpole tab", () => {
  test("Tadpole: refresh + verify metrics", async ({ page }) => {
...
await step("Switch to Tadpole and verify selected", async () => {
  ...
  await dash.waitForMetricsHeading(); 
    ...
    await step("Verify CSRO Dashboard - Tadpole header visible", async () => {
    ...
    await step("Verify 'PAX Traveler Status Metrics - Tadpole' is visible", async () => {
      const frame = await dash.getDashboardFrame();
      await expect(frame.getByText("PAX Traveler Status Metrics - Tadpole", { exact: true }))
